<template>
  <div class="oneCard-right">
    <div class="UserAssets-right-top">
			<div class="user-left">
				<span class="user-word">结算报表</span>
			</div>
    ...
    <div style="height: 700px;">
			<div style="height: 680px; margin-top: 20px;">
				<table class="tableClass">
					<tr class="firstTr">
						<td class="td1">日期</td>
						<td class="td2">商户</td>
						<td class="td2">订单总数</td>
						<td class="td2">订单金额</td>
						<td class="td2">分成金额</td>
						<td class="td2-1" colspan="2">
							<table style="width: 100%;">
								<tr style="width: 100%;">
									<td class="tddiv01" colspan="2">金额分布</td>
								</tr>
								<tr style="width: 100%;" class="tddiv22">
									<td class="tddiv02">笔笔消金额</td>
									<td class="tddiv02">转账金额</td>
								</tr>
							</table>
						</td>
						<td class="td2">钱包分成</td>
						<td class="td2">图表</td>
					</tr>
					<tr class="dataTable" v-for="item in cardList">
						<td class="data-height">{{item.dt}}</td>
						<td>{{item.username}}</td>
    ...
    methods: {
      send() {
			...
			first(){
				...
				// console.log(toKen)
				this.$axios.get("/admin/api/report/CCC86C79D64211EAB961FA163EC6B4B1?token=" + toKen + "&page=" + this.pagenum + "&row=10")
					.then(res => {
						if (res.status == 200) {
      ...
      daybao(id){
				...
				if(this.chooseId == 1){
					...
					let toKen = this.token.replace(/\"/g, "")
					// console.log(toKen)
					this.$axios.get("/admin/api/report/913B61BDD64411EAB961FA163EC6B4B1?token=" + toKen + "&page=" + this.pagenum + "&row=10")
						.then(res => {
							if (res.status == 200) {
								this.cardList = res.data.data //用户列表数据
								this.total = res.data.total
					...
					let toKen = this.token.replace(/\"/g, "")
					// console.log(toKen)
					this.$axios.get("/admin/api/report/92694694D64411EAB961FA163EC6B4B1?token=" + toKen + "&page=" + this.pagenum + "&row=10")
						.then(res => {
							if (res.status == 200) {
								this.cardList = res.data.data //用户列表数据
								this.total = res.data.total
